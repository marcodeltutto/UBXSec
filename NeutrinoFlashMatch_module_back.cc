...
NeutrinoFlashMatch::NeutrinoFlashMatch(fhicl::ParameterSet const & p)
{
  ...
  produces< art::Assns<anab::FlashMatch,   recob::PFParticle>>();
}
...
void NeutrinoFlashMatch::produce(art::Event & e)
{
  ...
  std::unique_ptr< std::vector<anab::FlashMatch>>                   flashMatchTrackVector      (new std::vector<anab::FlashMatch>);
  std::unique_ptr< art::Assns<anab::FlashMatch, recob::Track>>      assnOutFlashMatchTrack     (new art::Assns<anab::FlashMatch, recob::Track>     );
  std::unique_ptr< art::Assns<anab::FlashMatch, recob::PFParticle>> assnOutFlashMatchPFParticle(new art::Assns<anab::FlashMatch, recob::PFParticle>);
  ...
  for(_matchid=0; _matchid < (int)(_result.size()); ++_matchid) {
    ...
    flashMatchTrackVector->emplace_back(_score[_matchid], 0, _result.size(), true);
